# Purpose-Oriented Roadmap

**From parallel program optimization to custom-chip deep learning inference**

This document defines the **career progression** that this curriculum supports. The broad 5-phase structure (digital foundations → hardware platforms → acceleration → AI & edge → specialization) is re-framed here as **four purpose-oriented steps**. Each step is a concrete role target; the phases are the building blocks.
...
*This purpose-oriented roadmap aligns the curriculum with the roles: **parallel program optimization engineer** → **deep learning inference optimization engineer** → **DL inference for edge/AV/robotics** → **FPGA/custom chip for DL inference**. tinygrad and openpilot are the through-line reference projects.*
