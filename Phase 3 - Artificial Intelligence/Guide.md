...
Standalone phase after **Phase 1** (digital + HDL, architecture, OS, C++/CUDA) and **Phase 2** (PCB, MCU/RTOS, embedded Linux): learn **what** networks compute, **where** they run on-device, and **how** classic vision pipelines work before you map workloads to **Phase 4 Track A** (Xilinx FPGA) or **Track B** (Jetson).
...
**Suggested order:** Neural networks (theory + tinygrad) → Edge AI (deployment context) → Computer vision (or CV in parallel once tensors are familiar).
